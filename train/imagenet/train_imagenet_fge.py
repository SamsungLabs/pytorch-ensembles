...
def main_worker(gpu, ngpus_per_node, args):
    ...
    def adjust_learning_rate_(optimizer, epoch, batch_idx):
        t = ((epoch % 2)*epoch_batches + batch_idx) / (2.*epoch_batches)
        alpha_1 = 1e-3
        alpha_2 = 1e-5
        if t < 0.5:
            lr = alpha_2 * (1.0 - 2.0 * t) + alpha_1 * 2.0 * t
        else:
            lr = alpha_2 * (2.0 * t - 1.0) + alpha_1 * (2.0 - 2.0 * t)
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed
        ...
        path_ = (path_ + '/ImageNet-FGE_ResNet50-%s.pth.tar') % (epoch+1)
        print('\nEpoch %s acc %s lr %s Save model to %s\n' % (
            epoch,  acc1.item(),
            str([param_group['lr'] for param_group in optimizer.param_groups]), path_))
