...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, lv_init=-1, var_p=-1):
    """3x3 convolution with padding"""
    return vi.BayesConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=False, dilation=dilation, lv_init=lv_init, var_p=var_p)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lv_init=-1, var_p=-1):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv1x1(inplanes, width, lv_init=lv_init, var_p=var_p)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation, lv_init=lv_init, var_p=var_p)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(width, planes * self.expansion, lv_init=lv_init, var_p=var_p)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, lv_init=-1, var_p=-1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expansion, stride, lv_init=lv_init, var_p=var_p),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, lv_init=lv_init, var_p=var_p))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, lv_init=lv_init, var_p=var_p))
...
def BayesResNet50(pretrained=False, progress=True, **kwargs):
